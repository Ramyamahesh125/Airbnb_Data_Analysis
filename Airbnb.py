...
with col2 :
    img = Image.open(r"C:\Users\ramya\Downloads\Airbnb_logo.png")
...
def datafr():
    df= pd.read_csv(r"C:\Users\ramya\Airbnb.csv")
    return df

df= datafr()

with st.sidebar:
    select= option_menu("Main Menu", ["Home", "Data Exploration", "Geospatial Visualization","Top Charts"],
                        icons=['house', 'box','map','star'], menu_icon="cast", default_index=1)


if select == "Home":

    image= Image.open(r"C:\Users\ramya\Downloads\Airbnb_image.png")
    st.image(image)

    st.header(":blue[Airbnb:]")
    st.write("")
    ...
    st.header(":blue[Project Scope and Components]")
...
if select == "Data Exploration":
    ...
    with tab1:
        st.title(":violet[Price Difference]")
        col1,col2= st.columns(2)
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv(r"C:\Users\ramya\Airbnb.csv")
            return df_a

        df_a= datafr()

        st.title(":blue[Availability Analysis]")
        col1,col2= st.columns(2)

        with col1:
            
            
            country_a= st.selectbox("Select the Country To Check Availability",df_a["country"].unique())
    ...
    with tab3 :
        ...
        property_ty_l = st.selectbox("Select The Property Type :  ", df1_l["property_type"].unique())

        df2_l = df1_l[df1_l["property_type"] == property_ty_l ]
...
if select == "Geospatial Visualization":
    ...
    st.title(":violet[Geospatial Visualization]")
    st.write("")

    fig_4 = px.scatter_mapbox(df, lat='latitude', lon='longtitude', color='price', size='accommodates',
                    color_continuous_scale=px.colors.sequential.Rainbow,hover_name='name',range_color=(0,49000), mapbox_style="carto-positron",
                    zoom=1)
